...
enum ControlTableItemAddr{
  ...
  ADDR_ULTRASONIC_FRONT   = 194,
  ADDR_ULTRASONIC_RIGHT   = 198,
...
void TurtleBot3Core::begin(const char* model_name)
{
  ...
  // Initialize ultrasonic sensors and LED
  UltrasonicBegin();
  setupRgbPins();
  DEBUG_PRINTLN("Ultrasonic sensors and LED setup completed.");
  ...
  dxl_slave.addControlItem(ADDR_ULTRASONIC_FRONT, control_items.ultrasonic_front);
  dxl_slave.addControlItem(ADDR_ULTRASONIC_RIGHT, control_items.ultrasonic_right);
...
void TurtleBot3Core::run()
{
  ...
  // Update ultrasonic sensors and LED
  UltrasonicSpinOnce();
  updateRgbFromUltrasound();
...
void update_analog_sensors(uint32_t interval_ms)
{
  ...
  if(millis() - pre_time >= interval_ms){
    ...
    // Update ultrasonic sensor data from Ultrasonic.cpp
    control_items.ultrasonic_left = us[0].last_m;   // 좌측 센서
    control_items.ultrasonic_front = us[1].last_m;  // 전방 센서
    control_items.ultrasonic_right = us[2].last_m;  // 우측 센서
  }
}
